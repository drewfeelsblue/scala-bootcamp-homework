...
object LinkedList extends App {
  ...
  def findGap(l: List[Int]): Option[(Int, Int)] = l match {
    case Nil => None
    case l   => l zip l.tail find { case (a, b) => b - a != 1 }
  }
  ...
  def min(list: List[Int]): Option[Int] = {
    @tailrec
    def minRec(l: List[Int], om: Option[Int] = None): Option[Int] = l match {
      case Nil     => om
      case x :: xs => minRec(xs, om.map(_ min x).orElse(Some(x)))
    }
    minRec(list)
  }
  ...
  // pass the interview
  def count(s: String): List[(Char, Int)] = s.toList match {
    case Nil      => List.empty
    case c :: Nil => List((c, 1))
    case h :: t =>
...
object Leetcode {
  ...
  def maximumWealth(accounts: List[List[Int]]): Int = accounts.map(_.sum).max
  ...
  def maxWidthOfVerticalArea(points: List[List[Int]]): Int = {
    val sortedV = points.map(_.head).sorted
    (sortedV zip sortedV.tail).map { case (a, b) => b - a }.max
  }
